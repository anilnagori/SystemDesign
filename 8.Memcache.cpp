...
Impplement memchane which support the following features:
...
- set(curtTime, key, value, ttl). Set the key-value pair in memcache with a time to live (ttl). 
  The key will be valid from curtTime to curtTime + ttl - 1 and it will be expired after ttl 
  seconds. if ttl is 0, the key lives forever until out of memory.
- delete(curtTime, key). Delete the key.
- incr(curtTime, key, delta). Increase the key's value by delta return the new value. Return 
  2147483647 if key does not exist.
- decr(curtTime, key, delta). Decrease the key's value by delta return the new value. Return 
  2147483647 if key does not exist.
